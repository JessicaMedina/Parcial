    ...
    <html lang="es">
        ...
        <body style="height: 100%;">
            ...
            <div class="tabla2" style="font-family: monospace; font-size: 20px; color:#16a09b ;  width: 50%s">
                ...
                ?>
                <center><h1>Reporte de Cocina</h1>
                  <?php  
                     $res = mysql_query("select sum(ingredientes) from platos");
                $row = mysql_fetch_array($res);
                echo 'Total Ingredientes:  '. $row[0]; //muestra el total de la suma
                    ?>
                    <BR></BR>
                          <?php  
                     $res = mysql_query("select sum(cantidad) from compras");
                $row = mysql_fetch_array($res);
                echo 'Total Platos:  '. $row[0]; //muestra el total de la suma
                    ?>
                    <?php
                    echo "<table>";
                    echo "<tr>";
